...
cat >$NETIP <<-EOF
uci set network.lan.ipaddr='192.168.1.252'                                    # IPv4 地址(openwrt后台地址)
uci set network.lan.netmask='255.255.255.0'                                 # IPv4 子网掩码
uci set network.lan.gateway='192.168.1.0'                                   # IPv4 网关
...
uci set system.@system[0].hostname='OpenWrt'                                # 修改主机名称为OpenWrt-12
#sed -i 's/\/bin\/login/\/bin\/login -f root/' /etc/config/ttyd              # 设置ttyd免帐号登录，如若开启，进入OPENWRT后可能要重启一次才生效
EOF

sed -i "s/OpenWrt / malink compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ           # 增加个性名字 ${Author} 默认为你的github帐号

sed -i '/CYXluq4wUazHjmCDBCqXF/d' $ZZZ                                                             # 设置密码为空

#sed -i 's/PATCHVER:=5.4/PATCHVER:=5.10/g' target/linux/x86/Makefile                               # x86机型,默认内核5.4，修改内核为5.10

sed -i 's#%D %V, %C#%D %V, %C Lede_X86_64#g' package/base-files/files/etc/banner               # 自定义banner显示
